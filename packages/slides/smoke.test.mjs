...
const globalTop = fs.readFileSync(new URL('./global-top.vue', import.meta.url), 'utf8')
...
test('example deck relies on theme-owned default addons', () => {
  for (const addon of themePkg.slidev.defaults.addons) {
    assert.ok(themePkg.dependencies[addon], `${addon} should be installed by the theme package`)
