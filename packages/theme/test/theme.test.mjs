...
const styleEntrypoint = fs.readFileSync(
	new URL("../styles/index.js", import.meta.url),
	"utf8",
);
...
test("package enables the Hoverkraft default addon stack", () => {
	...
	for (const addon of defaultAddons) {
		assert.ok(
			pkg.dependencies[addon],
			`${addon} should be declared as a theme dependency`,
		);
	}
});
...
test("package ships the Hoverkraft logo components", () => {
	...
	for (const component of ["HoverkraftIcon", "HoverkraftLogo"]) {
		...
		assert.match(
			source,
			/<svg[\s\S]*<\/svg>/,
			`${component} should inline an SVG`,
		);
		assert.match(
			source,
			/aria-label/,
			`${component} should expose an accessible label`,
		);
	}
});
